...
*/
void Model_Loader::Load_All_Static_Meshes() {
	//All static models that aren't in RAM!
	...
	for (Mesh_File* mesh_file : Mesh_File::Get_All_OBJs()) {
		...
		for (Static_Model* model : Static_Model::ALL_MODELs) {
			if (mesh_file->NAME == model->Get_Name()) {
				cout << "Model: " << mesh_file->NAME << " is already uploaded to RAM!\n";
				Load_Model = false;
				break;
			}
		}
		//If Model is already loaded, skip the mesh!
		if (!Load_Model) {
			continue;
		}
	...
	//Add new loaded models to all models list's end!
	Static_Model::ALL_MODELs.insert(Static_Model::ALL_MODELs.end(), ALL_Static_Models.begin(), ALL_Static_Models.end());
	cout << "Static Model Number: " << Static_Model::ALL_MODELs.size() << endl;
}
...
void Create_Meshes(const aiScene* Scene, const vector<vector<unsigned int>>* MeshList_per_Material, Static_Model* Imported_Model, string name, string Directory) {
