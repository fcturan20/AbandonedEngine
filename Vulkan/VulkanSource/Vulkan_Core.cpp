...
namespace Vulkan {
	Vulkan_Core::Vulkan_Core() : FirstTriangle_CommandBuffers(LASTUSEDALLOCATOR, 10, 10), VK_States() {
	...
	void Vulkan_Core::Create_Window(const char* name) {
		...
		GLFWwindow* glfw_window = glfwCreateWindow(1280, 720, name, NULL, nullptr);
		WINDOW* Vulkan_Window = new WINDOW(1280, 720, GFX_API::WINDOWED, CONNECTED_Monitors[0], CONNECTED_Monitors[0]->REFRESH_RATE, name, GFX_API::V_SYNC_OFF);
		...
		ONSCREEN_Windows.push_back(Vulkan_Window);
	}
	...
	void Vulkan_Core::Initialization() {
		...
		Setup_Debugging();
#endif
		Create_Surface_forWindow((WINDOW*)ONSCREEN_Windows[0]);
		Check_Computer_Specs();
		Setup_LogicalDevice();
		Create_SwapChain_forWindow((WINDOW*)ONSCREEN_Windows[0]);
		TuranAPI::File_System::Material_Type_Resource FirstTriangle_MatType;
		FirstTriangle_MatType.NAME = "FirstTriangle_MatType";
		FirstTriangle_MatType.GFX_API = TuranAPI::TuranAPI_ENUMs::VULKAN;
		Create_ShaderPipeline(FirstTriangle_ShaderPipeline, &FirstTriangle_MatType);
		Create_RenderPass();
		Create_GraphicsPipeline();
		Create_Framebuffers((WINDOW*)ONSCREEN_Windows[0]);
		Create_CommandPool();
		Create_CommandBuffers();
		Begin_RenderPass();
		Begin_DrawingFirstTriangle();
		Finish_DrawingProgresses();
	...
	void Vulkan_Core::Create_Instance() {
		...
		VkExtensionProperties* SupportedExtensions = new VkExtensionProperties[extension_count];
		vkEnumerateInstanceExtensionProperties(nullptr, &extension_count, SupportedExtensions);
	...
	void Vulkan_Core::Check_Computer_Specs() {
		...
		Physical_GPU_LIST.resize(GPU_NUMBER);
		...
		for (unsigned int i = 0; i < GPU_NUMBER; i++) {
			GPU* Vulkan_GPU = new (LASTUSEDALLOCATOR) GPU;
	...
	void Vulkan_Core::Create_ShaderPipeline(ShaderPipeline* Pipeline_to_Create, TuranAPI::File_System::Material_Type_Resource* MaterialType_to_Use) {
		ShaderPipeline* ShaderPipe = new ShaderPipeline;
		if (MaterialType_to_Use->GFX_API != TuranAPI::TuranAPI_ENUMs::VULKAN) {
		...
		ci.pCode = reinterpret_cast<const uint32_t*>
			(TuranAPI::File_System::FileSystem::SELF->Read_BinaryFile("C:/dev/TuranEngine/Vulkan/Content/FirstShaderVert.spv", vertexcode_size));
		...
		fragmentci.pCode = reinterpret_cast<const uint32_t*>
			(TuranAPI::File_System::FileSystem::SELF->Read_BinaryFile("C:/dev/TuranEngine/Vulkan/Content/FirstShaderFrag.spv", fragmentcode_size));
	...
	void Vulkan_Core::Create_RenderPass() {
		...
		Attachment_Description.format = ((WINDOW*)ONSCREEN_Windows[0])->Window_SwapChainCreationInfo.imageFormat;
		...
		RenderPass_CreationInfo.dependencyCount = 0;
		RenderPass_CreationInfo.pDependencies = nullptr;
	...
	void Vulkan_Core::Create_CommandBuffers() {
		...
		if (vkBeginCommandBuffer(FirstTriangle_CommandBuffers[0], &CommandBuffer_bi) != VK_SUCCESS) {
	...
	void Vulkan_Core::Begin_RenderPass() {
		...
		RenderPass_bi.framebuffer = ((WINDOW*)ONSCREEN_Windows[0])->Swapchain_Framebuffers[0];
		RenderPass_bi.pNext = nullptr;
		RenderPass_bi.clearValueCount = 1;
		RenderPass_bi.pClearValues = &ClearColor_Value;

		vkCmdBeginRenderPass(FirstTriangle_CommandBuffers[0], &RenderPass_bi, VK_SUBPASS_CONTENTS_INLINE);

		TuranAPI::LOG_STATUS("Finished beginning Render Pass!");
	}
	void Vulkan_Core::Begin_DrawingFirstTriangle() {
		TuranAPI::LOG_STATUS("Started to send First Triangle's drawing commands!");

		vkCmdBindPipeline(FirstTriangle_CommandBuffers[0], VK_PIPELINE_BIND_POINT_GRAPHICS, FirstTriangle_GraphicsPipeline);
		vkCmdDraw(FirstTriangle_CommandBuffers[0], 3, 1, 0, 0);

		TuranAPI::LOG_STATUS("Finished sending First Triangle's drawing commmands!");
	}
	void Vulkan_Core::Finish_DrawingProgresses() {
		TuranAPI::LOG_STATUS("Started to close Rendering Operations!");

		vkCmdEndRenderPass(FirstTriangle_CommandBuffers[0]);
		vkEndCommandBuffer(FirstTriangle_CommandBuffers[0]);
	...
	void Vulkan_Core::Close_Window(GFX_API::WINDOW* window) {
		...
		for (unsigned int i = 0; i < Vulkan_Window->SwapChain_ImageViews.size(); i++) {
			vkDestroyImage(Vulkan_GPU->Logical_Device, Vulkan_Window->SwapChain_Images[i], nullptr);
		...
		vkDestroySwapchainKHR(((GPU*)GPU_TO_RENDER)->Logical_Device, Vulkan_Window->Window_SwapChain, nullptr);
		//vkDestroySurfaceKHR(Vulkan_States::Vulkan_Instance, Vulkan_Window->Window_Surface, nullptr);
