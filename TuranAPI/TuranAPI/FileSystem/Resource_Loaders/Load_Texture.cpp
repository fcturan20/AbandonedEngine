...
Resource_Type* FileSystem::Load_Texture(void* data, unsigned int id, const char* path, IAllocator* Allocator) {
...
void Save_a_Texture_toDisk(Resource_Type* Texture_Data) {
	...
	std::cout << "Exporting resource as a .texturecont: " << TEXTURE->PATH << std::endl;
...
unsigned int Sizeof_Real_ValueType(TuranAPI::TuranAPI_ENUMs value_type) {
	...
	case TuranAPI::TuranAPI_ENUMs::VAR_FLOAT32:
		return 32;
	default:
		std::cout << "Error: Intended size of Real Valuetype isn't supported for now!\n";
		SLEEP_THREAD(5);
		return 0;
	}
}
